...
class CoreSplitterTests(unittest.TestCase):
    def test_split_cluster_returns_local_assignments(self):
        ...
        assignments, new_ids = split_cluster(7, list(range(len(points))), points, 100)

        self.assertEqual(len(new_ids), 2)
        self.assertEqual(sorted(set(assignments.values())), sorted(new_ids))
        self.assertTrue(0 in assignments)
